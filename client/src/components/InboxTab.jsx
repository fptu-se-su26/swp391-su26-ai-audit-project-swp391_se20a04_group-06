...
export function InboxTab({ user }) {
  ...
  // Thêm tham số showLoading để tránh gọi setState đồng bộ trong useEffect
  // Khôi phục hàm kéo dữ liệu thuần túy, không chứa setLoading đồng bộ
  ...
  // Khởi chạy kéo dữ liệu ban đầu không kích hoạt setLoading đồng bộ
  useEffect(() => {
    loadConversations();
  }, []);

  // src/components/InboxTab.jsx (Trích đoạn hàm openChat)
  const openChat = (conv) => {
    setActiveChat(conv);
    ...
    // Kích hoạt đồng bộ hóa Navbar badge
    ...
    >
      ...
      <div>
        ...
        >
          ...
          {/* Nút bấm làm mới hộp thư */}
          <button
            onClick={() => {
              setLoading(true); // 🌟 Bật trạng thái quay vòng tải thủ công an toàn tại đây
              loadConversations(); // Tiến hành kéo dữ liệu mới
            }}
            style={{
              background: C.white,
      ...
      {activeChat && (
        <div style={{ position: "sticky", top: 80, alignSelf: "flex-start" }}>
          ...
          <ChatBox
            key={`${activeChat.productId}-${activeChat.otherUserId}`}
            product={{
