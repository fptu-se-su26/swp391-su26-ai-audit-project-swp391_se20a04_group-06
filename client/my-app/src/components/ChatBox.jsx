...
export function ChatBox({ product, onClose, user }) {
  ...
  useEffect(() => {
    api(`/messages/${product.id}`)
      .then((data) => {
        ...
        setMsgs(mapped);
        // Xác định người còn lại từ lịch sử tin nhắn
  ...
  useEffect(() => {
    ...
    function handleNewMessage(msg) {
      ...
      // Cập nhật otherUser nếu nhận tin từ người khác
      if (msg.senderId !== user.id) {
        otherUserRef.current = msg.senderId;
      }
      setMsgs((prev) => {
        // Tránh trùng lặp
  ...
  const send = () => {
    ...
    // Buyer gửi cho seller, seller gửi cho người đã nhắn (otherUser)
    const receiverId =
      user.id === sellerId ? otherUserRef.current : sellerId;
    if (!receiverId) {
