...
async function runBenchmark() {
  ...
  for (let i = 0; i < userCount; i++) {
    ...
    await sleep(200); // Giãn cách đăng ký để tránh rate limiter (max 5/hr/IP)
  }
      ...
      try {
        switch (scenarioType) {
          ...
          case 6: {
            ...
            const resDel = await request('POST', `/favorites/${product.id}`, null, user.cookies);
            const diffDel = process.hrtime(startDel);
            const msDel = (diffDel[0] * 1e9 + diffDel[1]) / 1e6;
            logMetric('DELETE_FAVORITE', msDel, resDel.statusCode === 200 || resDel.statusCode === 201, resDel.statusCode);
            break;
          }
        }
      } catch (err) {
