...
exports.messageService = {
    ...
    async getMessages(productId, userId, role, buyerIdStr) {
        ...
        if (role !== "Admin") {
            ...
            if (!prod)
                throw new HttpError_1.HttpError(404, "Sản phẩm không tồn tại");
            ...
            if (!buyerId) {
                throw new HttpError_1.HttpError(400, "Thiếu thông tin người mua (buyerId)");
            }
            ...
            await message_repository_1.messageRepository.markAsRead(productId, partnerId, userId);
        }
        else if (buyerIdStr) {
        ...
        const messages = await message_repository_1.messageRepository.find(filter, { path: "senderId", select: "name" }, { createdAt: 1 });
    ...
    async sendMessage(userId, data) {
        ...
        const { productId, receiverId, content, imageUrl, location, replyTo } = data;
        // Chặn người dùng tự gửi tin nhắn cho chính mình
        if (receiverId === userId) {
            throw new HttpError_1.HttpError(400, "Không thể tự gửi tin nhắn cho chính mình");
        }
        // Tin nhắn phải chứa ít nhất một trong các dữ liệu: chữ viết, hình ảnh, hoặc vị trí GPS
        if (!content?.trim() && !imageUrl && !location) {
            throw new HttpError_1.HttpError(400, "Tin nhắn không được rỗng");
        }
        ...
        if (prod.status === "Deleted") {
            throw new HttpError_1.HttpError(400, "Không thể gửi tin nhắn cho sản phẩm đã bị xóa");
        }
        ...
        try {
            ...
            logger_1.logger.error("Error emitting socket message from REST API:", err);
        }
        // Trả về bản ghi tin nhắn mới vừa lưu
        return newMsg;
    },
    // Nghiệp vụ lấy danh sách hội thoại chat (hộp thư inbox) của người dùng hiện tại
    async getConversations(userId, skip = 0, limit = 50) {
        // Gọi repository thực hiện truy vấn nhóm tổng hợp (Aggregation) để lấy danh sách hội thoại
        const conversations = await message_repository_1.messageRepository.getConversationAggregation(userId, skip, limit);
