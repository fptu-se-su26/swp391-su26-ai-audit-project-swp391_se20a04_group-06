// Import giao diện IImageUploader từ tầng UseCase để tuân thủ nguyên lý đảo ngược phụ thuộc (Dependency Inversion Principle)
  ...
  async uploadAvatar(buffer: Buffer): Promise<string> {
    ...
    const result: any = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { folder: "avatars" },  // Lưu trữ ảnh đại diện của người dùng trong thư mục 'avatars' trên Cloudinary
