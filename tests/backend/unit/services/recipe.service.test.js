...
const recipe_service_1 = require("../../../../backend/src/services/recipe.service");
// Import đối tượng recipeRepository để thiết lập giả lập các tương tác cơ sở dữ liệu công thức
const recipe_repository_1 = require("../../../../backend/src/repositories/recipe.repository");
...
describe("Unit Test: Recipe Service (recipe.service.ts)", () => {
    ...
    describe("Nghiệp vụ create (Tạo công thức món ăn)", () => {
        ...
        it("Nên chặn nếu người dùng thường chưa xác minh tài khoản cố tình tạo công thức", async () => {
            ...
            recipe_service_1.recipeService.create(mockUserId, "User", recipeData)).rejects.toThrow(
            ...
            expect.objectContaining({
                ...
                message: "Chỉ Admin hoặc ngư dân đã xác minh mới được viết công thức nấu ăn",
            }));
        });
        ...
        it("Nên cho phép tạo công thức thành công đối với ngư dân đã được xác minh danh tính", async () => {
            ...
            recipe_repository_1.recipeRepository.create.mockResolvedValue({
            ...
            const result = await recipe_service_1.recipeService.create(mockUserId, "User", recipeData);
