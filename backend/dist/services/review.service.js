...
exports.reviewService = {
    ...
    async addReview(reviewerId, body, fileBuffer) {
        ...
        if (reviewerId.toString() === sellerId.toString()) {
            ...
            throw new HttpError_1.HttpError(400, "Bạn không thể tự đánh giá chính mình");
        }
        ...
        const hasInteracted = await review_repository_1.reviewRepository.hasBuyerInteracted(
        ...
        if (!hasInteracted) {
            ...
            throw new HttpError_1.HttpError(403, "Chỉ những người đã liên hệ người bán về sản phẩm này mới được đánh giá");
        }
        // Kiểm tra tính duy nhất: Mỗi sản phẩm mẻ hàng người mua chỉ được phép đánh giá tối đa một lần duy nhất
        const existing = await review_repository_1.reviewRepository.existsByReviewerAndProduct(
        // ID người mua
        reviewerId, 
        // ID mẻ hàng
        productId);
        ...
        if (existing) {
            ...
            throw new HttpError_1.HttpError(409, "Bạn đã đánh giá sản phẩm này rồi");
        }
        ...
        await (0, notification_service_1.notifySellerNewReview)({
    ...
    async listSellerReviews(sellerId, skip, limit) {
        // Truy vấn cơ sở dữ liệu thông qua repository để lấy danh sách đánh giá và tổng số dòng khớp
        const { rows, total } = await review_repository_1.reviewRepository.findBySeller(
        // ID người bán nhận đánh giá
        sellerId, 
