...
export function VideoCallProvider({ children }) {
        ...
        const handleCallAccepted = async ({ answer }) => {
            ...
            try {
                ...
                if (pc) {
                    ...
                    console.log(`[WebRTC] Đang đồng bộ ${iceQueueRef.current.length} ứng viên mạng từ hàng đợi...`);
    ...
    const createPeerConnection = useCallback((targetId) => {
        ...
        pc.ontrack = (event) => {
            if (event.streams && event.streams[0]) {
                // Luôn bọc trong một MediaStream mới để ép React kích hoạt re-render
    ...
    const startCall = async (targetUserId, targetUserName) => {
        try {
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                throw new Error("Trình duyệt yêu cầu HTTPS bảo mật để sử dụng máy ảnh & micro.");
            }
    ...
    const acceptCall = async () => {
        ...
        try {
            ...
            socketRef.current.emit("answer_call", { to: currentPartnerId, answer: answerToSend });
            setCallState("connected");
    ...
    return (
        <VideoCallContext.Provider value={{ callState, partnerId, partnerName, startCall, acceptCall, endCall }}>
            {children}
            {callState !== "idle" && (
                <VideoCallOverlay
                    callState={callState}
...
export function useVideoCall() {
    ...
    if (!ctx) throw new Error("useVideoCall phải được sử dụng bên trong VideoCallProvider");
    return ctx;
}