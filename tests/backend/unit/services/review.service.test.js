...
const review_service_1 = require("../../../../backend/src/services/review.service");
...
describe("Unit Test: Nghiệp vụ Review Service (review.service.ts)", () => {
    ...
    it("Nên ném lỗi 400 nếu người dùng tự gửi đánh giá cho chính bản thân mình", async () => {
        ...
        review_service_1.reviewService.addReview(mockReviewerId, body)).rejects.toThrow(
    ...
    it("Nên ném lỗi 403 nếu người mua chưa từng nhắn tin trao đổi với người bán về mẻ hàng này", async () => {
        ...
        review_repository_1.reviewRepository.hasBuyerInteracted.mockResolvedValue(false);
        ...
        expect.objectContaining({
            ...
            message: "Chỉ những người đã liên hệ người bán về sản phẩm này mới được đánh giá",
        }));
    });
    ...
    it("Nên ném lỗi 409 nếu người mua cố tình gửi đánh giá lần thứ 2 cho cùng một sản phẩm", async () => {
        ...
        review_repository_1.reviewRepository.hasBuyerInteracted.mockResolvedValue(true);
        // Giả lập đã tồn tại bản ghi đánh giá của cặp (reviewerId, productId) này trong cơ sở dữ liệu (trả về true)
        review_repository_1.reviewRepository.existsByReviewerAndProduct.mockResolvedValue(true);
    ...
    it("Nên lưu đánh giá thành công khi đáp ứng đầy đủ điều kiện ràng buộc nghiệp vụ", async () => {
        ...
        review_repository_1.reviewRepository.existsByReviewerAndProduct.mockResolvedValue(false);
        // Giả lập hàm lưu đánh giá của reviewRepository trả về đối tượng chứa ID đánh giá giả lập
        review_repository_1.reviewRepository.create.mockResolvedValue({
        ...
        const result = await review_service_1.reviewService.addReview(mockReviewerId, body);
        ...
        expect(review_repository_1.reviewRepository.create).toHaveBeenCalledWith(
