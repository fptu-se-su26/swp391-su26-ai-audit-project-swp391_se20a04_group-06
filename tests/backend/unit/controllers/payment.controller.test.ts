import {
  ...
  getPremiumStatus,
  sepayWebhook,
} from "../../../../backend/src/controllers/payment.controller";
import { userRepository } from "../../../../backend/src/repositories/user.repository";
import { PaymentTransaction } from "../../../../backend/src/models/PaymentTransaction";
...
describe("Premium payment endpoints", () => {
  ...
  it("returns a VietQR intent when Sepay bank variables are configured", async () => {
    ...
    await getPremiumIntent(request, response);
  ...
  it("does not expose a fake QR when bank variables are absent", async () => {
    ...
    await getPremiumIntent(
      { user: { userId: "60c72b2f9b1d8b2bad000001" } } as any,
      response,
    );
  ...
  it("returns premium state and latest transaction", async () => {
    ...
    (PaymentTransaction.findOne as jest.Mock).mockReturnValue({ sort });
    const response = createResponse();
