...
// Import đối tượng giá trị GPSCoordinates đại diện cho tọa độ vĩ độ và kinh độ của sản phẩm
import { GPSCoordinates } from "../value-objects/GPSCoordinates";

// Định nghĩa giao diện (interface) ProductProps chứa các thuộc tính cốt lõi của một sản phẩm
export interface ProductProps {
  // Mã định danh của người bán (seller) sở hữu sản phẩm này
  sellerId: string;
  // Loại sản phẩm, chỉ nhận một trong hai giá trị là "Fresh" (tươi sống) hoặc "Dried" (đồ khô)
  type: "Fresh" | "Dried";
  // Danh mục phân loại sản phẩm (ví dụ: cá, tôm, cua, mực...)
  category: string;
  // Tên hiển thị của sản phẩm
  name: string;
  // Bài viết hoặc nội dung mô tả chi tiết về sản phẩm
  description: string;
  ...
  totalWeight: number;
  // Khối lượng còn lại của sản phẩm sau khi đã bán (đơn vị kg)
  remainingWeight: number;
  // Trạng thái hiện tại của sản phẩm, gồm: Active (Đang bán), SoldOut (Hết hàng), Expired (Hết hạn), Deleted (Đã xóa)
  status: "Active" | "SoldOut" | "Expired" | "Deleted";
  // Vị trí GPS hiện tại hoặc vị trí lưu kho của sản phẩm (không bắt buộc với đồ khô)
  location?: GPSCoordinates;
  // Vị trí GPS nơi sản phẩm được đánh bắt (dành cho hải sản tươi sống)
  catchLocation?: GPSCoordinates;
  // Thời gian sản phẩm được đánh bắt ngoài khơi (dành cho hải sản tươi sống)
  catchTime?: Date;
  // Nguồn gốc xuất xứ của sản phẩm (ví dụ: Vịnh Bắc Bộ, Phú Quốc, Khánh Hòa...)
  origin?: string;
