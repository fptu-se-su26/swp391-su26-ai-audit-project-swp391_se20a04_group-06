// Import interface định nghĩa các phương thức giao tiếp với Database của User (Repository Pattern)
...
export class GoogleAuthUseCase {
  ...
  async execute(idToken: string, selectedRole?: "buyer" | "seller") {
    ...
    if (isMockToken) {
      ...
      logger.info(`🔑 [MOCK GOOGLE LOGIN] Email=${email}, Name=${name}`); // Ghi nhận log sự kiện đăng nhập giả lập thành công
    } 
    // KỊCH BẢN 2: ĐĂNG NHẬP BẰNG GOOGLE OAUTH 2.0 THỰC TẾ
      ...
      logger.info(`✅ [GOOGLE SIGN IN SUCCESS] Email=${email}, Name=${name}`); // Ghi nhận log đăng nhập Google thành công
    }
    ...
    if (!user) {
      ...
      logger.info(`✨ Created new Google User: ID=${user.id}, Email=${email}, Role=${role}, IsVerified=${isVerified}`); // Ghi nhận log đã tạo người dùng thành công
    } 
    // KỊCH BẢN B: NGƯỜI DÙNG ĐÃ TỒN TẠI (ĐĂNG NHẬP LẠI)
      ...
      if (!rawProps.isGoogleLinked) {
        ...
        needsSave = true;
        logger.info(`🔗 Auto-linked existing User account with Google: Email=${email}`);
      }
      ...
      logger.info(`🚪 Existing Google User logged in: ID=${user.id}, Email=${email}`); // Ghi nhận log đăng nhập thành công người dùng cũ
    }
