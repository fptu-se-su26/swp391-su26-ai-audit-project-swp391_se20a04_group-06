...
const Entity_1 = require("../../../../../backend/src/shared/domain/Entity");
// Import lớp đối tượng giá trị ValueObject làm cơ sở để kế thừa
const ValueObject_1 = require("../../../../../backend/src/shared/domain/ValueObject");
// Import lớp gốc AggregateRoot làm cơ sở để kiểm thử các sự kiện miền
const AggregateRoot_1 = require("../../../../../backend/src/shared/domain/AggregateRoot");
// Import lớp quản lý sự kiện miền DomainEvents để điều phối các sự kiện trong kiểm thử
const DomainEvents_1 = require("../../../../../backend/src/shared/domain/events/DomainEvents");
...
class MockEntity extends Entity_1.Entity {
}
// Khởi tạo một Value Object giả lập MockValueObject kế thừa từ lớp ValueObject gốc
class MockValueObject extends ValueObject_1.ValueObject {
...
class MockAggregate extends AggregateRoot_1.AggregateRoot {
...
describe("DDD Shared Kernel - DomainEvents", () => {
    ...
    it("nên đăng ký được handler và kích hoạt nó khi gửi sự kiện", () => {
        ...
        DomainEvents_1.DomainEvents.register((event) => {
        ...
        DomainEvents_1.DomainEvents.dispatchEventsForAggregate(aggregate.id);
