...
class UpdateProductUseCase {
    ...
    async execute(id, userId, role, body) {
        ...
        if (role !== "Admin" && product.sellerId !== userId) {
            ...
            throw new DomainException_1.UnauthorizedError("Bạn không có quyền chỉnh sửa bài đăng này");
        }
        ...
        if (targetType === "Fresh") {
            ...
            if (targetLat == null || targetLng == null) {
                ...
                throw new DomainException_1.ValidationError("Tọa độ GPS vị trí mẻ hàng là bắt buộc đối với hải sản tươi sống!");
            }
            // Khởi tạo đối tượng GPSCoordinates từ vĩ độ và kinh độ đích đã được ép kiểu số thực
            location = GPSCoordinates_1.GPSCoordinates.create(parseFloat(targetLat), parseFloat(targetLng));
        }
        else {
        ...
        if (targetCatchLat != null && targetCatchLng != null) {
            ...
            catchLocation = GPSCoordinates_1.GPSCoordinates.create(parseFloat(targetCatchLat), parseFloat(targetCatchLng));
        }
        // 2. Cập nhật cân nặng
        ...
        if (body.images !== undefined && Array.isArray(body.images)) {
            ...
            if (removedImages.length > 0) {
                ...
                const removedPublicIds = removedImages.map(cloudinary_2.extractPublicId).filter((id) => !!id);
                ...
                if (removedPublicIds.length > 0) {
                    ...
                    cloudinary_1.cloudinary.api.delete_resources(removedPublicIds).catch((err) => {
        ...
        await redis_1.redis.del(`product:detail:${id}`).catch(() => { });
