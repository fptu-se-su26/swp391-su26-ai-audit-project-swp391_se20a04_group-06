...
const SITE_NAME  = 'Chợ Hải Sản Online | HảiSản.vn';
const SITE_URL   = typeof window !== 'undefined' ? window.location.origin : 'https://haisanvn.vn';
 ...
 */
export function useSEO({ title, description, image, url, product, seller } = {}) {
  useEffect(() => {
    const fullTitle = title ? `${title} | HảiSản.vn` : SITE_NAME;
    const desc      = description || 'Chợ hải sản tươi sống & khô trực tiếp từ ngư dân. Mua tôm, cá, mực tươi giao nhanh 20km.';
    ...
    setMeta('og:site_name',   'HảiSản.vn', true);
    ...
    if (product) {
      ...
      setJsonLd('jsonld-product', {
        ...
        offers: {
          ...
          availability: parseFloat(product.remainingWeight) > 0
      ...
      setJsonLd('jsonld-product', {
        '@context': 'https://schema.org',
        '@type': 'WebSite',
        name: 'HảiSản.vn — Chợ Hải Sản Online',
        url: SITE_URL,
        description: desc,
        potentialAction: {
          '@type': 'SearchAction',
