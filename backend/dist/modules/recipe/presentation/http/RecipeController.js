...
const MongooseRecipeRepository_1 = require("../../infrastructure/persistence/mongoose/MongooseRecipeRepository");
// Import lớp CreateRecipeUseCase để thực hiện nghiệp vụ tạo mới công thức nấu ăn
const CreateRecipeUseCase_1 = require("../../application/use-cases/CreateRecipeUseCase");
// Import lớp UpdateRecipeUseCase để thực hiện nghiệp vụ cập nhật thông tin công thức nấu ăn
const UpdateRecipeUseCase_1 = require("../../application/use-cases/UpdateRecipeUseCase");
// Import lớp DeleteRecipeUseCase để thực hiện nghiệp vụ xóa công thức nấu ăn
const DeleteRecipeUseCase_1 = require("../../application/use-cases/DeleteRecipeUseCase");
// Import lớp ToggleLikeRecipeUseCase để thực hiện nghiệp vụ thích hoặc bỏ thích công thức nấu ăn
const ToggleLikeRecipeUseCase_1 = require("../../application/use-cases/ToggleLikeRecipeUseCase");
// Khởi tạo đối tượng Repository dùng chung cho các Use Cases
const recipeRepository = new MongooseRecipeRepository_1.MongooseRecipeRepository();
// Khởi tạo Use Case tạo mới công thức, tiêm Repository vào qua Constructor (DI)
const createRecipeUseCase = new CreateRecipeUseCase_1.CreateRecipeUseCase(recipeRepository);
// Khởi tạo Use Case cập nhật công thức, tiêm Repository vào qua Constructor (DI)
const updateRecipeUseCase = new UpdateRecipeUseCase_1.UpdateRecipeUseCase(recipeRepository);
// Khởi tạo Use Case xóa công thức, tiêm Repository vào qua Constructor (DI)
const deleteRecipeUseCase = new DeleteRecipeUseCase_1.DeleteRecipeUseCase(recipeRepository);
// Khởi tạo Use Case thích/bỏ thích công thức, tiêm Repository vào qua Constructor (DI)
const toggleLikeRecipeUseCase = new ToggleLikeRecipeUseCase_1.ToggleLikeRecipeUseCase(recipeRepository);
...
async function toggleLikeRecipe(req, res, next) {
    // Trích xuất và định dạng ID công thức từ tham số URL params
    const id = (0, response_helper_1.parseId)(req.params.id);
