...
export function PostListingPage() {
  ...
  const submit = async () => {
    ...
    // Chuyển đổi dữ liệu sang dạng số để kiểm tra phạm vi
    const parsedPrice = parseFloat(price);
    const parsedWeight = parseFloat(weight);

    if (isNaN(parsedPrice) || parsedPrice < 1) {
      return setErr("Giá bán phải từ 1 VND/kg trở lên và không được âm");
    }

    if (isNaN(parsedWeight) || parsedWeight < 0.1) {
      return setErr(
        "Khối lượng hải sản phải từ 0.1 kg trở lên và không được âm",
      );
    }
      ...
      >
        ...
        >
          ...
          <p style={{ color: C.muted, fontSize: 14, fontWeight: 500 }}>
            Hải sản của bạn đã được xuất bản lên trang chủ Haisan.vn.
          </p>
          <button
            onClick={() => navigate("/")}
            style={{
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "32px 64px 80px" }}>
      ...
      {!postCount.loading && !postCount.isPremium && (
        <div
          style={{
            background:
              postCount.count >= postCount.max ? "#FEE2E2" : "#EFF6FF",
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div
              style={{
                fontWeight: 800,
                fontSize: 13.5,
                color: postCount.count >= postCount.max ? "#991B1B" : "#1E3A8A",
              }}
            >
              {postCount.count >= postCount.max
          ...
          <p
            style={{
              fontSize: 13,
              color: postCount.count >= postCount.max ? "#7F1D1D" : "#1E40AF",
              margin: 0,
              lineHeight: 1.5,
              fontWeight: 500,
            }}
          >
            {postCount.count >= postCount.max
              ...
              : `Bạn đang là thành viên thường. Hãy nâng cấp lên Premium để mở khoá tính năng đăng bán không giới hạn!`}
          </p>
          <button
            type="button"
      ...
      {!postCount.loading && postCount.isPremium && (
        ...
        >
          ...
          <div>
            ...
            >
              Đặc quyền đăng bán tin hải sản không giới hạn số lượng bài viết
              mỗi ngày đã sẵn sàng hoạt động.
            </p>
          </div>
        </div>
      )}
      ...
      >
        ...
        >
          {[
            ["Fresh", "Hải sản tươi", C.coral, "#FDE8E0"],
            ["Dried", "Hải sản khô", C.warn, "#FEF5E4"],
          ].map(([k, l, ac, bg]) => (
            <button
              key={k}
              onClick={() => setType(k)}
        ...
        >
          ...
          >
            {gps.status === "ok"
              ...
              : "⚠️ Hải sản tươi cần phải bật định vị để khoanh vùng 20km"}
          </span>
          {gps.status !== "ok" && (
            <button
              onClick={getGps}
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <input
                ...
                type="number"
                min="0.1" // Thêm dòng này
                step="0.1" // Thêm dòng này
              />
            </div>
          </div>
