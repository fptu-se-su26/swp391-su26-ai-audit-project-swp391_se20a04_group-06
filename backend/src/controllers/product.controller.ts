...
export async function getProducts(req: Request, res: Response) {
  try {
    ...
    const fetchOffset = (type === 'Fresh' && lat && lng) ? 0 : (pageNum - 1) * limitNum;
...
export async function getProductById(req: Request, res: Response) {
  ...
  try {
    ...
    await pool.query(`UPDATE Product SET ViewCount = ViewCount + 1 WHERE ProductID = ?`, [id]).catch(() => {});
  ...
  try {
    const [result] = await pool.query<ResultSetHeader>(
      `INSERT INTO Product
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        userId, type, name.trim(), description ?? null, parseInt(price),
        salesType ?? 'Retail', parseFloat(totalWeight),
        parseFloat(totalWeight), // RemainingWeight = TotalWeight lúc mới đăng
        catchTime ?? null, lat ?? null, lng ?? null, origin ?? null, expiryDate ?? null,
      ],
    );
    const productId = result.insertId;

    // Lấy tên người bán rồi phát thông báo (delegated to service)
    const [userRows] = await pool.query<RowDataPacket[]>('SELECT Name FROM User WHERE UserID = ?', [userId]);
  ...
  try {
    ...
    await pool.query(
      `UPDATE Product SET
       ...
       WHERE ProductID = ?`,
      [name, description, price, salesType, remainingWeight, status, origin, expiryDate, id],
    );
...
export async function bumpProduct(req: Request, res: Response) {
  ...
  try {
    ...
    if (product.SellerID !== userId) return res.status(403).json({ message: 'Không có quyền' });
    ...
    if (product.BumpedAt) {
      ...
      if (diffMs < 24 * 3600 * 1000) {
        ...
        return res.status(429).json({ message: `Đẩy tin lại sau ${remaining} giờ nữa` });
      }
    }
    // Bump = cập nhật BumpedAt (dùng làm sort key)
    await pool.query(
      `UPDATE Product SET BumpedAt = NOW() WHERE ProductID = ?`, [id]
    );
