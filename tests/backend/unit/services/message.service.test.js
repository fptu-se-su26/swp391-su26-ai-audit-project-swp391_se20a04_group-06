...
const message_service_1 = require("../../../../backend/src/services/message.service");
// Import đối tượng messageRepository để mock tác vụ lưu trữ tin nhắn
const message_repository_1 = require("../../../../backend/src/repositories/message.repository");
...
describe("Unit Test: Message Service (message.service.ts)", () => {
    ...
    describe("Nghiệp vụ sendMessage (Gửi tin nhắn)", () => {
        ...
        it("Nên chặn gửi tin nhắn nếu người nhận trùng với người gửi", async () => {
            ...
            await expect(message_service_1.messageService.sendMessage(mockUserId, body)).rejects.toThrow(expect.objectContaining({
                status: 400,
        ...
        it("Nên dọn dẹp thẻ HTML độc hại trong nội dung và tạo tin nhắn thành công", async () => {
            ...
            const body = {
                ...
                content: "<div>Cá thu <b>còn tươi</b> không bạn? <script>alert('xss')</script></div>",
                imageUrl: "hinh_anh_ca.png",
            };
            // Giả lập hàm create lưu trữ tin nhắn trả về bản ghi có content đã được dọn sạch
            message_repository_1.messageRepository.create.mockResolvedValue({
                _id: "msg_123",
                content: "Cá thu còn tươi không bạn? alert('xss')",
                imageUrl: "hinh_anh_ca.png",
            });
            // Thực thi hàm gửi tin nhắn
            const result = await message_service_1.messageService.sendMessage(mockUserId, body);
            ...
            expect(message_repository_1.messageRepository.create).toHaveBeenCalledWith(expect.objectContaining({
                productId: mockProductId,
                senderId: mockUserId,
                receiverId: mockReceiverId,
