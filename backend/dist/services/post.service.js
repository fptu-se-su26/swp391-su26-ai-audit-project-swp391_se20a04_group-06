...
exports.postService = {
    ...
    async list(query) {
        ...
        if (query.userId && (0, response_helper_1.parseId)(query.userId)) {
        ...
        const { posts, total } = await post_repository_1.postRepository.findAll(filter, skip, limit, sortByScore);
    ...
    async getById(id) {
        ...
        const post = await post_repository_1.postRepository.findByIdAndIncrementView(id);
        // Nếu không tồn tại bài viết tương ứng, ném lỗi 404
        if (!post)
            throw new HttpError_1.HttpError(404, "Không tìm thấy bài đăng");
        // Trả về chi tiết bài đăng
        return post;
    },
    // Tạo mới một bài viết chia sẻ trên cộng đồng diễn đàn
    async create(userId, // ID tác giả đăng bài
    data) {
        ...
        (0, badge_service_1.updateUserBadges)(userId).catch(() => { });
    ...
    async toggleLike(postId, userId) {
        ...
        if (index === -1) {
            ...
            updatedPost = await post_repository_1.postRepository.addLike(postId, userId);
            liked = true; // Thiết lập trạng thái đã thích
        }
        else {
            // Ngược lại, gọi repository để hủy thích
            updatedPost = await post_repository_1.postRepository.removeLike(postId, userId);
        }
    ...
    async addComment(postId, userId, text) {
        ...
        const updatedPost = await post_repository_1.postRepository.addComment(postId, {
            userId,
    ...
    async delete(postId, userId, role) {
        ...
        if (role !== "Admin" && post.userId.toString() !== userId) {
            throw new HttpError_1.HttpError(403, "Bạn không có quyền xóa bài đăng này");
        }
        // Gọi repository thực hiện xóa bài đăng khỏi cơ sở dữ liệu
        await post_repository_1.postRepository.delete(postId);
    },
    // Nghiệp vụ xóa một bình luận của bài đăng
    async deleteComment(postId, // ID bài viết chứa bình luận
    commentId, // ID bình luận cần xóa
    userId, // ID người yêu cầu xóa
    role) {
        // Tìm kiếm bài đăng theo ID
        const post = await post_repository_1.postRepository.findById(postId);
        // Nếu không tồn tại bài viết tương ứng, ném lỗi 404
        ...
        if (!comment)
            throw new HttpError_1.HttpError(404, "Không tìm thấy bình luận");
        // Chặn quyền: Chỉ cho phép Admin, tác giả của bình luận, HOẶC tác giả của bài viết được phép xóa
        if (role !== "Admin" &&
            comment.userId.toString() !== userId &&
            post.userId.toString() !== userId) {
            throw new HttpError_1.HttpError(403, "Bạn không có quyền xóa bình luận này");
        }
        // Gọi repository thực hiện gỡ bình luận ra khỏi mảng comments
        const updatedPost = await post_repository_1.postRepository.deleteComment(postId, commentId);
