...
async function updateUserBadges(userId) {
    try {
        ...
        const productCount = await product_repository_1.productRepository.countDocuments({
            sellerId: userIdStr,
            status: "Active",
        });
        ...
        const hasSquid = await product_repository_1.productRepository.findOne({
            sellerId: userIdStr,
            category: "Squid",
            status: "Active",
        });
        ...
        const postCount = await post_repository_1.postRepository.countDocuments({
            userId: userIdStr,
        });
        ...
        const writtenReviewsCount = await review_repository_1.reviewRepository.countDocuments({
        ...
        await user_repository_1.userRepository.updateBadges(userIdStr, badges);
        // Trả về danh sách huy hiệu đạt được
        return badges;
    }
    catch (err) {
        // Ghi log lỗi nếu quá trình tính toán hoặc lưu trữ huy hiệu bị trục trặc
        logger_1.logger.error(`Error updating user badges for ${userId}: ${err.message}`);
        // Trả về mảng trống
        return [];
    }
}
