...
class CreateProductUseCase {
    ...
    async execute(userId, body) {
        ...
        const { type, // Loại sản phẩm ("Fresh" - Tươi sống hoặc "Dried" - Đồ khô)
        ...
        if (type === "Fresh") {
            ...
            location = GPSCoordinates_1.GPSCoordinates.create(parseFloat(lat), parseFloat(lng));
        }
        else {
        ...
        if (catchLat != null && catchLng != null) {
            ...
            catchLocation = GPSCoordinates_1.GPSCoordinates.create(parseFloat(catchLat), parseFloat(catchLng));
        }
        ...
        if (!user)
            throw new DomainException_1.ValidationError("Không tìm thấy người dùng");
        ...
        if (!user.isPremium && user.role !== "Admin") {
            ...
            if (currentCount > 5) {
                ...
                throw new DomainException_1.ConflictError("Tài khoản thường chỉ được phép đăng tối đa 5 bài viết mỗi ngày. Vui lòng nâng cấp lên Premium để đăng không giới hạn!");
            }
        }
        ...
        if (isNaN(parsedPrice) || isNaN(parsedWeight)) {
            ...
            throw new DomainException_1.ValidationError("Thông tin giá cả hoặc khối lượng không hợp lệ");
        }
        ...
        await redis_1.redis.incr(`product:list:version:${type}`).catch(() => { });
        // Gửi thông báo đến những người dùng đang theo dõi (followers) về việc có sản phẩm mới
        (0, notification_service_1.notifyFollowersNewProduct)(userId, // ID của người bán
        user.name, // Tên của người bán
        product.id, // ID sản phẩm vừa tạo
        product.name // Tên sản phẩm vừa tạo
        ).catch((err) => logger_1.logger.error(`[Notify] notifyFollowersNewProduct failed: ${err.message}`)); // Ghi nhật ký lỗi nếu gửi thông báo thất bại
