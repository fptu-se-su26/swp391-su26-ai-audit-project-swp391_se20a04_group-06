...
exports.favoriteService = {
    ...
    async getMyFavorites(userId) {
        ...
        const user = await user_repository_1.userRepository.findFavoritesPopulated(userId);
    ...
    async toggleFavorite(userId, productId) {
        ...
        if (!productId || !mongoose_1.default.Types.ObjectId.isValid(productId)) {
            throw new HttpError_1.HttpError(400, "ID không hợp lệ");
        }
        ...
        if (!isFavorited) {
            ...
            const productExists = await product_repository_1.productRepository.exists({
                _id: productId,
                status: { $ne: "Deleted" },
            });
            ...
            if (!productExists) {
                // Ném lỗi 404
                throw new HttpError_1.HttpError(404, "Sản phẩm không tồn tại hoặc đã bị xóa");
            }
            // Gọi repository để thêm ID sản phẩm này vào danh sách yêu thích của người dùng
            await user_repository_1.userRepository.addFavorite(userId, productId);
        ...
        await user_repository_1.userRepository.removeFavorite(userId, productId);
