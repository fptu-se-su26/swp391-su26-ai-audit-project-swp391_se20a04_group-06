...
async function redisGet(key) {
    try {
        return await withTimeout(redis_1.redis.get(key));
    }
    catch {
        logger_1.logger.warn(`[Redis] GET failed: ${key}`);
        return null;
    }
}
...
async function redisSet(key, value, mode, ttl) {
    try {
        ...
        const p = mode && ttl ? redis_1.redis.set(key, value, mode, ttl) : redis_1.redis.set(key, value);
        await withTimeout(p);
    }
    catch {
        logger_1.logger.warn(`[Redis] SET failed: ${key}`);
    }
}
// Tăng giá trị của một khóa trong Redis (dùng để quản lý số phiên bản version list nhằm xóa cache hàng loạt)
async function redisIncr(key) {
    try {
        await withTimeout(redis_1.redis.incr(key));
    }
    catch {
        logger_1.logger.warn(`[Redis] INCR failed: ${key}`);
    }
}
// Xóa một khóa trong Redis
async function redisDel(key) {
    try {
        await withTimeout(redis_1.redis.del(key));
    }
    catch {
        logger_1.logger.warn(`[Redis] DEL failed: ${key}`);
    }
}
...
exports.productService = {
    ...
    async list(query) {
        ...
        const listVersion = queryType === "Fresh"
            ? freshVer
            : queryType === "Dried"
                ? driedVer
                : `${freshVer}_${driedVer}`;
        ...
        const { page, limit } = (0, pagination_1.parsePagination)(rawPage, rawLimit);
        ...
        if (sellerId && mongoose_1.default.Types.ObjectId.isValid(sellerId)) {
            filter.sellerId = new mongoose_1.default.Types.ObjectId(sellerId);
        }
        ...
        const sortOption = { bumpedAt: -1, createdAt: -1 };
        const projection = {};
            ...
            if (!isNaN(latVal) && !isNaN(lngVal)) {
                filter.location = {
                    $geoWithin: {
                        ...
                        $centerSphere: [[lngVal, latVal], haversine_1.MAX_FRESH_DISTANCE_KM / 6378.1],
                    },
                };
            }
        }
        ...
        const rows = await product_repository_1.productRepository.find(filter, projection, {
            skip,
            limit,
            sort: sortOption,
        });
        ...
        const finalResponse = (0, pagination_1.paginatedResponse)(formattedRows, total, page, limit);
    ...
    async getById(id) {
        ...
        if (!mongoose_1.default.Types.ObjectId.isValid(id)) {
            throw new HttpError_1.HttpError(400, "ID sản phẩm không hợp lệ");
        }
        ...
        if (cachedDetail) {
            try {
                ...
                const rawViews = await withTimeout(redis_1.redis.incr(viewsCacheKey).catch(() => null));
                if (rawViews !== null)
                    parsed.viewCount = rawViews;
        ...
        const p = await product_repository_1.productRepository.findByIdAndUpdate(id, { $inc: { viewCount: 1 } }, { new: true });
        // Nếu không tìm thấy sản phẩm hoặc sản phẩm đã bị xóa
        if (!p || p.status === "Deleted") {
            throw new HttpError_1.HttpError(404, "Không tìm thấy sản phẩm hoặc sản phẩm đã bị xóa");
        }
    ...
    async getProducts(sellerId, pageStr, limitStr) {
    ...
    async getTodayCount(userId) {
        ...
        const countStr = await redis_1.redis.get(limitKey);
    ...
    async create(userId, // ID người bán
    body) {
        const { type, category, name, description, price, salesType, totalWeight, catchTime, lat, lng, origin, expiryDate, images, } = body;
        // Yêu cầu bắt buộc phải truyền tọa độ GPS vị trí mẻ hàng đối với hải sản tươi sống (Fresh)
        if (type === "Fresh" && (lat == null || lng == null)) {
            throw new HttpError_1.HttpError(400, "Tọa độ vị trí GPS là bắt buộc đối với hải sản tươi sống!");
        }
        ...
        if (!user.isPremium && user.role !== "Admin") {
            ...
            const currentCount = await redis_1.redis.incr(limitKey);
            // Thiết lập thời gian hết hạn khóa đếm là 24 giờ nếu đây là lượt đăng đầu tiên
            if (currentCount === 1) {
                await redis_1.redis.expire(limitKey, 24 * 3600);
            }
            // Nếu số lượng vượt quá mức cho phép (5 bài đăng)
            if (currentCount > 5) {
                // Thực hiện giảm lại số lượng để hoàn tác và ném lỗi 403 từ chối quyền
                await redis_1.redis.decr(limitKey);
                throw new HttpError_1.HttpError(403, "Tài khoản thường chỉ được phép đăng tối đa 5 bài viết mỗi ngày. Vui lòng nâng cấp lên Premium để đăng không giới hạn!");
            }
        }
        ...
        if (isNaN(parsedPrice) || isNaN(parsedWeight)) {
            ...
            // Ném lỗi 400
            throw new HttpError_1.HttpError(400, "Thông tin giá cả hoặc khối lượng không hợp lệ");
        }
        ...
        const catchCoordinates = body.catchLat != null && body.catchLng != null
            ? [
        ...
        try {
            ...
            savedProduct = await product_repository_1.productRepository.create({
                sellerId: userId,
                type,
                category,
                name: name.trim(),
        ...
        (0, badge_service_1.updateUserBadges)(userId).catch((err) => {
            logger_1.logger.error(`[Badge Award Error] Không thể cập nhật danh hiệu cho UserID=${userId}: ${err.message}`);
        });
        ...
        user_repository_1.userRepository
            .findRawById(userId)
            .then((seller) => (0, notification_service_1.notifyFollowersNewProduct)(userId, seller?.name || "Một ngư dân", savedProduct._id.toString(), savedProduct.name))
            .catch((err) => logger_1.logger.error(`[Notify] notifyFollowersNewProduct failed: ${err.message}`));
        // Trả về ID sản phẩm tạo thành công
        return { productId: savedProduct._id.toString() };
    },
    // Nghiệp vụ cập nhật thông tin sản phẩm
    async update(id, // ID sản phẩm
    userId, // ID người cập nhật
    role, // Vai trò người cập nhật
    body) {
        ...
        const currentProduct = await product_repository_1.productRepository.findById(id);
        // Nếu không tồn tại sản phẩm, ném lỗi 404
        if (!currentProduct)
            throw new HttpError_1.HttpError(404, "Không tìm thấy sản phẩm");
        ...
        if (role !== "Admin" && currentProduct.sellerId.toString() !== userId)
            throw new HttpError_1.HttpError(403, "Bạn không có quyền chỉnh sửa bài đăng này");
        // Tính toán tổng khối lượng mới và khối lượng còn lại mới
        const finalTotalWeight = body.totalWeight !== undefined
            ? parseFloat(body.totalWeight)
            : currentProduct.totalWeight;
        const finalRemainingWeight = body.remainingWeight !== undefined
        ...
        if (finalRemainingWeight > finalTotalWeight) {
            throw new HttpError_1.HttpError(400, "Khối lượng còn lại không thể lớn hơn tổng khối lượng của mẻ hàng.");
        }
        ...
        const targetLat = body.lat !== undefined
            ? body.lat
            : currentProduct.location?.coordinates?.[1];
        const targetLng = body.lng !== undefined
            ? body.lng
        ...
        if (targetType === "Fresh" && (targetLat == null || targetLng == null)) {
            throw new HttpError_1.HttpError(400, "Tọa độ GPS vị trí mẻ hàng là bắt buộc đối với hải sản tươi sống!");
        }
        // Ép kiểu giá bán
        const newPrice = body.price !== undefined ? parseInt(body.price, 10) : undefined;
        if (newPrice !== undefined && isNaN(newPrice)) {
            throw new HttpError_1.HttpError(400, "Giá sản phẩm mới không hợp lệ");
        }
        ...
        if (body.origin !== undefined)
            updateFields.origin = body.origin;
        if (body.type !== undefined)
            updateFields.type = body.type;
        if (body.category !== undefined)
            updateFields.category = body.category;
        if (body.salesType !== undefined)
            updateFields.salesType = body.salesType;
        ...
        if (body.images !== undefined && Array.isArray(body.images)) {
            ...
            if (removedImages.length > 0) {
                ...
                const removedPublicIds = removedImages
                    .map(cloudinary_2.extractPublicId)
                    .filter((id) => !!id);
                if (removedPublicIds.length > 0) {
                    // Gọi API Cloudinary xóa ảnh để giải phóng không gian lưu trữ
                    cloudinary_1.cloudinary.api
                        .delete_resources(removedPublicIds)
                        .catch((err) => {
                        logger_1.logger.error(`Cloudinary cleanup failed during update: ${err.message}`);
                    });
                }
            }
        }
        ...
        await product_repository_1.productRepository.findByIdAndUpdate(id, updateQuery);
        // Tính toán cập nhật lại danh hiệu ngư dân
        (0, badge_service_1.updateUserBadges)(currentProduct.sellerId).catch((err) => {
            logger_1.logger.error(`[Badge Award Error] Không thể cập nhật danh hiệu cho UserID=${currentProduct.sellerId}: ${err.message}`);
        });
    ...
    async delete(id, userId, role) {
        ...
        if (role !== "Admin" && currentProduct.sellerId.toString() !== userId)
            throw new HttpError_1.HttpError(403, "Bạn không có quyền xoá bài đăng này");
        ...
        await product_repository_1.productRepository.findByIdAndUpdate(id, {
            $set: { status: "Deleted" },
        });
        ...
        await notification_repository_1.notificationRepository.deleteByProductId(id);
        await user_repository_1.userRepository.updateMany({}, { $pull: { favorites: id } });
        await report_repository_1.reportRepository.deleteByProductId(id);
        // Xóa cache chi tiết và tăng phiên bản trên Redis
        await redisDel(`product:detail:${id}`);
        await redisIncr(`product:list:version:${currentProduct.type}`);
    },
    // Nghiệp vụ đẩy bài đăng sản phẩm lên đầu trang tìm kiếm (cooldown 24h)
    async bump(id, userId) {
        ...
        if (currentProduct.sellerId.toString() !== userId)
            throw new HttpError_1.HttpError(403, "Không có quyền");
        ...
        const updated = await product_repository_1.productRepository.findOneAndUpdate({
            _id: id,
            sellerId: userId,
            $or: [
        ...
        if (!updated) {
            ...
            throw new HttpError_1.HttpError(429, `Sản phẩm này đã được đẩy lên gần đây. Vui lòng đẩy tin lại sau.`);
        }
