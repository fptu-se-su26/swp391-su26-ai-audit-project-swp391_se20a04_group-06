...
const boatLog_service_1 = require("../../../../backend/src/services/boatLog.service");
// Import đối tượng boatLogRepository để giả lập các phản hồi truy vấn nhật ký
const boatlog_repository_1 = require("../../../../backend/src/repositories/boatlog.repository");
...
jest.mock("../../../../backend/src/repositories/boatlog.repository");
...
describe("Unit Test: BoatLog Service (boatLog.service.ts)", () => {
    ...
    describe("Nghiệp vụ create (Đăng nhật ký cabin)", () => {
        ...
        it("Nên chặn nếu tài khoản thường chưa xác minh và chưa nâng cấp gói Premium", async () => {
            ...
            await expect(boatLog_service_1.boatLogService.create(mockUserId, logData)).rejects.toThrow(expect.objectContaining({
                status: 403,
        ...
        it("Nên cho phép đăng nhật ký cabin thành công nếu tài khoản đã mua gói Premium", async () => {
            ...
            boatlog_repository_1.boatLogRepository.create.mockResolvedValue({
                _id: mockLogId,
            });
            ...
            const result = await boatLog_service_1.boatLogService.create(mockUserId, logData);
