...
/**
 * Import kiểu TypeScript `Express`
 *
 * Express là framework web phổ biến nhất cho Node.js
 * Ở đây chỉ import kiểu dữ liệu (type) để TypeScript hiểu tham số
 * của hàm setupSwagger() sau này nhận vào là một ứng dụng Express
 *
 * 💡 Ghi nhớ: Trong TypeScript, `import { Express }` chỉ import kiểu,
 *    không import code thực thi — giúp code an toàn hơn khi biên dịch
 */
...
const options: swaggerJSDoc.Options = {
  ...
  definition: {
    ...
    info: {
       ...
       */
      description:
        "Tài liệu API chi tiết cho ứng dụng mua bán hải sản HảiSản.vn\n\n" +
...
/**
 * Export swaggerSpec ra ngoài module
 *
 * Tại sao cần export?
 * - Dùng trong unit test để kiểm tra tài liệu API có đúng không
 * - Dùng trong script tự động xuất file swagger.json
 * - Dùng trong CI/CD để validate tài liệu API trước khi deploy
 *
 * Cách dùng ở nơi khác:
 * ```typescript
 * import { swaggerSpec } from "./swagger";
 * // swaggerSpec là một object JSON theo chuẩn OpenAPI 3.0
 * console.log(swaggerSpec.info.version); // "1.0.0"
 * ```
 */
export { swaggerSpec };
