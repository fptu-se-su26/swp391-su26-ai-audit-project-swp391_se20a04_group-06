...
exports.recipeService = {
    ...
    async list(query) {
        ...
        if (query.authorId && (0, response_helper_1.parseId)(query.authorId)) {
        ...
        const { recipes, total } = await recipe_repository_1.recipeRepository.findAll(
        // Bộ lọc đã tổng hợp
        filter, 
    ...
    async getById(id) {
        ...
        const recipe = await recipe_repository_1.recipeRepository.findByIdAndIncrementView(id);
    ...
    async create(userId, role, data) {
        ...
        if (role !== "Admin" && !user.isVerified) {
            ...
            throw new HttpError_1.HttpError(403, "Chỉ Admin hoặc ngư dân đã xác minh mới được viết công thức nấu ăn");
        }
        // Thực hiện lưu trữ công thức mới vào cơ sở dữ liệu thông qua repository
        return recipe_repository_1.recipeRepository.create({
    ...
    async toggleLike(recipeId, userId) {
        ...
        const recipe = await recipe_repository_1.recipeRepository.findById(recipeId);
        ...
        if (index === -1) {
            ...
            updatedRecipe = await recipe_repository_1.recipeRepository.addLike(recipeId, userId);
            // Gán cờ trạng thái đã thích thành true
            liked = true;
        }
        else {
            // Ngược lại, nếu đã thích thì thực hiện xóa ID người dùng ra khỏi danh sách mảng thích
            updatedRecipe = await recipe_repository_1.recipeRepository.removeLike(recipeId, userId);
        }
    ...
    async update(recipeId, userId, role, data) {
        ...
        if (role !== "Admin" && recipe.authorId._id.toString() !== userId) {
            ...
            throw new HttpError_1.HttpError(403, "Bạn không có quyền chỉnh sửa công thức này");
        }
        ...
        if (data.cookingTime)
            updates.cookingTime = data.cookingTime;
        ...
        return recipe_repository_1.recipeRepository.update(recipeId, updates);
    },
    // Nghiệp vụ xóa công thức nấu ăn khỏi hệ thống
    async delete(recipeId, userId, role) {
        ...
        if (role !== "Admin" && recipe.authorId._id.toString() !== userId) {
            ...
            throw new HttpError_1.HttpError(403, "Bạn không có quyền xóa công thức này");
        }
        // Thực thi lệnh xóa công thức nấu ăn khỏi cơ sở dữ liệu qua repository
        await recipe_repository_1.recipeRepository.delete(recipeId);
    },
};
