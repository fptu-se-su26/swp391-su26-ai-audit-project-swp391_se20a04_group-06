...
describe("Unit Test: Nghiệp vụ Follow Service (follow.service.ts)", () => {
    ...
    it("Nên báo lỗi 400 nếu định dạng ID người bán không hợp lệ", async () => {
        ...
        await expect(follow_service_1.followService.toggleFollow(mockUserId, invalidId)).rejects.toThrow(expect.objectContaining({
            status: 400,
    ...
    it("Nên chặn không cho người dùng tự theo dõi chính bản thân", async () => {
        ...
        await expect(follow_service_1.followService.toggleFollow(mockUserId, mockUserId)).rejects.toThrow(expect.objectContaining({
            status: 400,
    ...
    it("Nên tiến hành theo dõi thành công khi chưa theo dõi và người bán tồn tại", async () => {
        ...
        user_repository_1.userRepository.isFollowing.mockResolvedValue(false);
        // Giả lập tài khoản người bán tồn tại đang hoạt động
        user_repository_1.userRepository.exists.mockResolvedValue(true);
        // Thực thi hàm toggleFollow
        const result = await follow_service_1.followService.toggleFollow(mockUserId, mockSellerId);
        ...
        expect(user_repository_1.userRepository.followSeller).toHaveBeenCalledWith(mockUserId, mockSellerId);
    });
    ...
    it("Nên tiến hành hủy theo dõi nếu người dùng đã bấm theo dõi người bán từ trước", async () => {
        ...
        user_repository_1.userRepository.isFollowing.mockResolvedValue(true);
        // Thực thi hàm toggleFollow
        const result = await follow_service_1.followService.toggleFollow(mockUserId, mockSellerId);
        ...
        expect(user_repository_1.userRepository.unfollowSeller).toHaveBeenCalledWith(mockUserId, mockSellerId);
    });
});
