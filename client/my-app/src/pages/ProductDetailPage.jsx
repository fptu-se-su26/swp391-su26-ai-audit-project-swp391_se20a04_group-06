  ...
  return (
    <div className={styles.gallery}>
      ...
      <div className={styles.mainImage}>
        ...
        <div className={styles.imgCounter}>📸 {idx + 1}/{n}</div>
...
export function ProductDetailPage({ product: initialProduct }) {
  ...
  const handleToggleFollow = () => {
    ...
    if (user.userId === product.sellerId) { toast.warn("Bạn không thể tự theo dõi chính mình!"); return; }
    setTogglingFollow(true);
    api(`/follows/${product.sellerId}/toggle`, { method: "POST" })
      .then((res) => { setIsFollowing(res.isFollowing); toast.success(res.message); })
        ...
        <>
              ...
              <div className={styles.descSection}>
                ...
                <table className={styles.infoTable}>
                  <tbody>
                    <tr>
                      <th>Phân loại</th>
                      <td>{CATEGORY_MAP[product.category] || "✨ Phân loại khác"}</td>
                    </tr>
                    <tr>
                      <th>Loại hình</th>
                      <td>{product.type === "Fresh" ? "🌊 Hải sản tươi sống" : "🔥 Hải sản khô / chế biến"}</td>
                    </tr>
                    {product.salesType && (
                      <tr>
                        <th>Kiểu bán</th>
                        <td>{product.salesType === "Wholesale" ? "📦 Bán sỉ buôn lô" : "🛒 Bán lẻ"}</td>
                      </tr>
                    )}
                    <tr>
                      <th>Khối lượng</th>
                      <td><strong>{product.totalWeight} kg</strong> (còn {product.remainingWeight} kg)</td>
                    </tr>
                    {product.origin && (
                      <tr>
                        <th>Xuất xứ</th>
                    ...
                    {product.catchTime && (
                      <tr>
                        <th>Thời gian bắt</th>
                        <td>⏰ {new Date(product.catchTime).toLocaleString("vi-VN")}</td>
                      </tr>
                    )}
                    {product.expiryDate && (
                      <tr>
            ...
            <div className={styles.infoArea}>
              ...
              <div className={styles.infoCard}>
                ...
                <div className={styles.badges}>
                  <span className={`${styles.badge} ${product.type === "Fresh" ? styles.badgeFresh : styles.badgeDried}`}>
                    {product.type === "Fresh" ? "🌊 Hải sản Tươi" : "🔥 Hải sản Khô"}
                  </span>
                  ...
                  {product.salesType === "Wholesale" && (
                    <span className={`${styles.badge} ${styles.badgeWholesale}`}>Bán sỉ</span>
                  )}
                </div>
                ...
                <div className={styles.priceBlock}>
                  ...
                  <span className={styles.priceTax}>税込 (đã bao gồm thuế)</span>
                </div>
                ...
                {priceHistory.length > 0 && (
                  <div style={{ marginBottom: 12 }}>
                    ...
                    >
                      📈 {showPriceHistory ? "Ẩn lịch sử đổi giá ▲" : "Xem lịch sử đổi giá ▼"}
                    </button>
                    {showPriceHistory && (
                      <div className={styles.priceHistoryPanel}>
                        ...
                        {priceHistory.map((h, i) => (
                          <div key={i} className={styles.priceHistoryItem}>
                            <div>
                              <span style={{ textDecoration: "line-through", color: "#94a3b8" }}>{fmt(h.oldPrice)}</span>
                              {" "}➔{" "}
                              <strong style={{ color: C.coral }}>{fmt(h.newPrice)}</strong>
                            </div>
                ...
                <div className={styles.stockSection}>
                  <div className={styles.stockHeader}>
                    <span>🐟 Còn lại <strong>{product.remainingWeight} kg</strong></span>
                    <span style={{ color: stockColor, fontWeight: 700 }}>{pct}%</span>
                  </div>
                  <div className={styles.stockBar}>
                    <div className={styles.stockFill} style={{ width: `${Math.min(100, pct)}%`, background: stockColor }} />
                  </div>
                </div>
                  ...
                  <div className={styles.sellerInfo}>
                    <div className={styles.sellerName}>
                      ...
                      {product.sellerIsPremium === 1 && (
                        <span title="Thành viên Premium uy tín" style={{ fontSize: 14, cursor: "help" }}>👑</span>
                      )}
                    </div>
                    <div className={styles.sellerCta}>Xem gian hàng ngư dân →</div>
                  </div>
                ...
                {product.sellerBadges && product.sellerBadges.length > 0 && (
                  <div className={styles.fishBadges}>
                    {product.sellerBadges.map((badge, idx) => (
                      <span key={idx} className={styles.fishBadge}>🎖️ {badge}</span>
                    ))}
                  </div>
                )}
                ...
                <div className={styles.ctaGroup}>
                  {/* Primary Order CTA (umai.fish 注文する style) */}
                  <button
                    className={styles.ctaOrder}
                    onClick={() => {
                      if (!user) { navigate("/dang-nhap"); return; }
                      setShowChat((v) => !v);
                    }}
                  >
                    🛒 Đặt hàng ngay
                    <span className={styles.ctaOrderArrow}>➔</span>
                  </button>
              ...
              <ul className={styles.highlightsList}>
                <div className={styles.highlightsHeader}>📌 Điểm nổi bật của mẻ hàng</div>
                <li className={styles.highlightsLi}>🐟 100% tự nhiên từ vùng biển sạch</li>
                <li className={styles.highlightsLi}>⏱️ Xử lý cấp đông đá tại tàu lập tức</li>
                <li className={styles.highlightsLi}>📦 Hút chân không, đóng thùng xốp sạch sẽ</li>
                <li className={styles.highlightsLi}>🚀 Giao nhanh xe đông lạnh bảo đảm độ tươi tuyệt đối</li>
              </ul>
            </div>
          </div>
          ...
          <div className={styles.detailsSection}>
            {images.slice(1).map((img, i) => (
              <div key={i} className={`${styles.detailRow} ${i % 2 === 0 ? styles.rowNormal : styles.rowReverse}`}>
                <div className={styles.detailImage}>
                  <img src={img.url} alt={`Mô tả chi tiết ${i + 1}`} />
                </div>
                <div className={styles.detailContent}>
                  <h3 className={styles.detailRowTitle}>
                    {i === 0 ? "⚓ Nguồn gốc và quy trình đánh bắt" : i === 1 ? "❄️ Quy trình bảo quản & đóng gói" : "🍽️ Hướng dẫn chế biến & thưởng thức"}
                  </h3>
                  <p className={styles.detailRowText}>
                    {paragraphs[i + 1] || "Sản phẩm được đánh bắt có trách nhiệm tại các ngư trường bản địa Việt Nam, xử lý cấp đông lạnh lập tức ngay trên tàu để đảm bảo giữ nguyên vị ngọt tự nhiên khi giao tới tay khách hàng."}
                  </p>
                </div>
              </div>
            ))}
            ...
            {images.length <= 1 && (
              <>
                <div className={`${styles.detailRow} ${styles.rowNormal}`}>
                  ...
                  <div className={styles.detailContent}>
                    <h3 className={styles.detailRowTitle}>⚓ Quy trình đánh bắt tự nhiên</h3>
                    <p className={styles.detailRowText}>
                      Hải sản được đánh bắt tự nhiên tại các vùng vịnh nước sâu Việt Nam. Chúng tôi cam kết khai thác bền vững, bảo vệ hệ sinh thái và nguồn lợi thủy hải sản địa phương.
                    </p>
                  </div>
                </div>
                <div className={`${styles.detailRow} ${styles.rowReverse}`}>
                  ...
                  <div className={styles.detailContent}>
                    <h3 className={styles.detailRowTitle}>❄️ Bảo quản đông đá ngay tại tàu</h3>
                    <p className={styles.detailRowText}>
                      Sau khi kéo lưới, hải sản được phân loại và ủ đá lạnh hoặc cấp đông sâu ngay trong khoang tàu cá. Quy trình đóng gói hút chân không kỹ lưỡng giúp ngăn ngừa vi khuẩn và giữ chất lượng đạt chuẩn tươi sống.
                    </p>
                  </div>
                </div>
              </>
          ...
          <div className={styles.producerCard}>
            ...
            <div className={styles.producerBody}>
              <div className={styles.producerLeft}>
                ...
                {product.sellerBadges && product.sellerBadges.length > 0 && (
                  <div className={styles.producerBadges}>
                    {product.sellerBadges.map((badge, idx) => (
                      <span key={idx} className={styles.producerBadge}>🎖️ {badge}</span>
                    ))}
                  </div>
                )}
              </div>
              ...
              <div className={styles.producerRight}>
                <h4 className={styles.producerQuote}>"Khát vọng mang hải sản tươi sạch từ biển cả đến bàn ăn mọi nhà"</h4>
                <p className={styles.producerBio}>
                  Là đối tác liên kết lâu năm của HảiSản.vn, chúng tôi cam kết đánh bắt có trách nhiệm, bảo vệ nguồn lợi thủy sản địa phương. Các mẻ lưới được phân loại và xử lý cấp đông sâu hoặc đóng đá lạnh ngay tại tàu cá cập cảng để giữ trọn vị ngon tinh khiết nhất.
                </p>
          ...
          <div className={styles.seasonalCard}>
            <div className={styles.producerHeader}>
              <div className={styles.sectionTitleAccent} />
              <h2>Mùa vụ đánh bắt của ngư thuyền / 季節ごとの水揚</h2>
            </div>
            <div className={styles.seasonalGrid}>
              <div className={`${styles.seasonBox} ${styles.spring}`}>
                <div className={styles.seasonTitle}>🌸 Mùa xuân (Tháng 3 - 5)</div>
                <p>Cá trích, cá bóp, tôm thẻ, cá hồng, mực cơm</p>
              </div>
              <div className={`${styles.seasonBox} ${styles.summer}`}>
                <div className={styles.seasonTitle}>☀️ Mùa hạ (Tháng 6 - 8)</div>
                <p>Tôm hùm, cá thu, ghẹ xanh, bạch tuộc, mực ống</p>
              </div>
              <div className={`${styles.seasonBox} ${styles.autumn}`}>
                <div className={styles.seasonTitle}>🍂 Mùa thu (Tháng 9 - 11)</div>
                <p>Cua gạch, cá cơm khô, sò điệp, mực lá, tôm sú</p>
              </div>
              <div className={`${styles.seasonBox} ${styles.winter}`}>
                <div className={styles.seasonTitle}>❄️ Mùa đông (Tháng 12 - 2)</div>
                <p>Cá ngừ đại dương, cá mú, cua huỳnh đế, bạch tuộc mắt tre</p>
              </div>
            </div>
          </div>
          ...
          <div className={styles.scheduleCard}>
            <div className={styles.producerHeader}>
              <div className={styles.sectionTitleAccent} />
              <h2>Lịch trình & Giao hàng dự kiến / 直近の出荷予定</h2>
            </div>
            <p className={styles.scheduleDesc}>
              Lịch trình đi biển và chuẩn bị đơn hàng của ngư thuyền trong tuần này. Giao hàng nhanh từ 24h - 48h sau khi thuyền cập cảng.
            </p>
            <div className={styles.tableWrapper}>
              <table className={styles.scheduleTable}>
                ...
                <tbody>
                  <tr>
                    <td className={styles.fishermanName}>🚢 Ngư thuyền {product.sellerName}</td>
                    {next7Days.map((d, i) => (
          ...
          <div className={styles.commitmentBanner}>
            🛡️ CAM KẾT CHẤT LƯỢNG: GIÁ HIỂN THỊ LÀ GIÁ TRỌN GÓI ĐÃ BAO GỒM VAT & MIỄN PHÍ VẬN CHUYỂN TOÀN QUỐC!
          </div>
          ...
          {showReportModal && (
            <div className={styles.modalOverlay} onClick={() => setShowReportModal(false)}>
              <div className={styles.modalCard} onClick={(e) => e.stopPropagation()}>
                ...
                {reportSent ? (
                  <div style={{ textAlign: "center", padding: "24px 0", color: "#10b981", fontWeight: 700, fontSize: 15 }}>
                  ...
                  <>
                    ...
                    <textarea
                      ...
                      onFocus={(e) => e.target.style.borderColor = "#0b4f6c"}
