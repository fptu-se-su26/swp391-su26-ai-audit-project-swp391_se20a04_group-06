import { Calendar, ChefHat, Clock3, Heart, Loader, Pencil, Plus, Search, Trash2, Users, X } from "lucide-react";
...
export default function Recipes() {
  ...
  const filteredRecipes = useMemo(() => {
    return recipes.filter((r) => {
      ...
      const matchDiff =
        ...
        (r.difficulty && r.difficulty.toLowerCase() === selectedDifficulty.toLowerCase());
  ...
  return (
    <div className="page-container recipes-page fb-page-layout">
      <div className="fb-layout-2col">
        {/* ── CỘT 1: LEFT SIDEBAR BỘ LỌC CÂM NANG (Chuẩn Facebook Marketplace) ── */}
        <aside className="fb-left-column fb-sticky-sidebar">
          <div className="fb-sidebar-card">
            <h3 className="fb-sidebar-title">
              <ChefHat size={20} /> Cẩm Nang Công Thức
            </h3>

            <div className="form-field" style={{ marginBottom: "14px" }}>
              <span style={{ fontSize: "0.82rem", fontWeight: 700, color: "#64748b" }}>Tìm kiếm món ăn</span>
            ...
            <div className="form-field" style={{ marginBottom: "14px" }}>
              <span style={{ fontSize: "0.82rem", fontWeight: 700, color: "#64748b" }}>Mức độ chế biến</span>
              <select
            ...
            <button
              ...
              type="button"
            >
              <Plus size={17} /> Chia sẻ công thức
            </button>
          </div>
        </aside>

        {/* ── CỘT 2: RIGHT MAIN CONTENT GRID ── */}
        <main className="fb-right-column">
          <header className="page-heading" data-tour="recipes-heading" style={{ marginBottom: "20px" }}>
            <div>
              <h1>Cẩm nang món ngon từ hải sản</h1>
              <p>Hơn 500+ công thức chế biến hải sản tươi sống từ cộng đồng ngư dân.</p>
            </div>
          </header>
          ...
          {!loading && filteredRecipes.length === 0 && (
            <div className="page-state" style={{ padding: "40px 0" }}>Không tìm thấy công thức nào phù hợp.</div>
          )}
